    ...
    for data in tqdm(dataloader):
        ...
        for name in names:
            ...
            for name in glob.glob(DataPath+batch+'/files/'+'/'+parser.metricType+'*'+series+'.csv'): 
                ...
                labels_tensor=(torch.from_numpy(values[1]))
        ...
        if array.shape[1]==parser.condition_len:
            ...
            #y_predicted= torch.nn.functional.softmax(y_predicted.to(device), dim=0)

            #division = torch.div(torch.stack(a), 100)
            y_truth =torch.stack(a).to(device)
            #y_truth= torch.nn.functional.softmax(y_truth, dim=1)
...
def metrics(criterion,y_predicted,y_truth, opt,running_loss,epoch_loss,acc_train,train=True):
    ...
    #score = torch.tensor(torch.sum(y_predicted==y_truth).item()/(100*parser.batch_size))
...
def main():
    ...
    fwd_test, opt, criterion,scheduler=get_net_resnet(device,hiden_num=1000,dropout=0.1,features=1000, Y_prediction_size=100)
    ...
    date="_RESNET152_Bands_17July_2e-5_100epc_h1000_f1000_64_CE"
