...
def arguments():
    ...
    parser.resnet_arch="resnet18" #this is to be modified when training for different metrics.
...
def get_data_with_labels(image_size, randomResize, imagesPath, dataset_path,batch_size, drop_last):
    ...
    targets = targets.replace("splitcross", 3)
    targets = targets.replace("ring",2 )
...
def get_net_resnet(device,hiden_num=1000,dropout=0.1,features=3000, Y_prediction_size=601):
    model = Stack.Predictor_RESNET_V2(parser.resnet_arch,conditional=True, cond_input_size=parser.condition_len, 
    ...
    opt = optimizer.Adam(model.parameters(), lr=parser.learning_rate, betas=(0.5, 0.98),weight_decay=1e-5)
...
# Conditioning
def set_conditioning(target,path,categories,df):
    ...
    for idx,name in enumerate(path):
        ...
        val_arr=torch.cat((val_arr,torch.Tensor([sustratoHeight,substrateWidth])),0)
...
def epoch_train(epoch,model,dataloader,validation_dataloader,device,opt,scheduler,criterion,clipEmbedder,df,run):
    i=0 #iteration
    running_loss = 0. 
    epoch_loss = 0.
    acc_train=0.0
    ...
    for data in tqdm(dataloader):
        ...
        array=set_conditioning(classes, names, classes_types,df)
        ...
        y_truth = torch.stack(aa).to(device)
    ...
    with torch.no_grad():
        ...
        inputs, classes, names, classes_types = data = next(iter(validation_dataloader))
        inputs = inputs.to(device) #images
        classes =classes.to(device) #categories
        ...
        counter=0
        for ax in axs:
            ax.plot(range(0,100), predicted[0][counter].cpu().detach().numpy(),color="salmon")
            ax.plot(range(0,100), aa[counter].cpu().detach().numpy(),"-0", color='royalblue')
            counter=counter+1

        plt.savefig("./"+run+"/"+batch+"_"+series+"_"+str(epoch)+".png")
        plt.clf()
        del y_predicted, predicted
...
def metrics(criterion,y_predicted,y_truth, opt,running_loss,epoch_loss,acc_train,train=True):
    ...
    loss_per_batch=0
    #apply criterion
    loss = criterion(y_predicted.float(), y_truth.float()) * 1000
    if train:
        loss.backward()
...
def train(opt,scheduler,criterion,model, clipEmbedder,device, PATH, run):
    #### #File reading conf
    ...
    for epoch in range(parser.epochs):
        ...
        total,epoch_loss,acc_train,score_train=epoch_train(epoch,
                                                           model,
                                                           dataloader,
                                                           vdataloader,
                                                           device,
...
def main():
    ...
    fwd_test, opt, criterion,scheduler=get_net_resnet(device,hiden_num=1000,dropout=0.4,features=600, Y_prediction_size=parser.pred_size)
    #fwd_test, opt, criterion,scheduler=get_net_CNN(device,hiden_num=1000,dropout=0.3,features=1000, Y_prediction_size=parser.pred_size)
    ...
    try:
        np.savetxt(run+'/loss_Train_TM_'+date+'.out', loss_values, delimiter=',')
    except:
        np.savetxt(run+'/loss_Train_TM_'+date+'.out', [], delimiter=',')

    try:
        np.savetxt(run+'/acc_Train_TM_'+date+'.out', acc, delimiter=',')
    except:
        np.savetxt(run+'/acc_Train_TM_'+date+'.out', [], delimiter=',')
    
    try:
        np.savetxt(run+'/loss_Valid_TM_'+date+'.out', valid_loss_list, delimiter=',')
    except:
        np.savetxt(run+'/loss_Valid_TM_'+date+'.out', [], delimiter=',')
    
    try:
        np.savetxt(run+'/acc_val_'+date+'.out', acc_val, delimiter=',')
    except:
        np.savetxt(run+'/acc_val_'+date+'.out', [], delimiter=',')

    try:
        np.savetxt(run+'/score_train_'+date+'.out', score_train, delimiter=',')
    except:
        np.savetxt(run+'/score_train_'+date+'.out', [], delimiter=',')
