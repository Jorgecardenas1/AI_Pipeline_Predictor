...
def arguments():
    ...
    parser.condition_len=2 #this is to be modified when training for different metrics.
...
def set_conditioning(bands_batch,freqx_val,target,path,categories,clipEmbedder,df,device):
    ...
    for idx,name in enumerate(path):
        ...
        #val_arr=torch.cat((val_arr,torch.Tensor([band,freqx_val])),0)
    ...
    for data in tqdm(dataloader):
        ...
        for name in names:
            ...
            for name in glob.glob(DataPath+batch+'/files/'+'/'+parser.metricType+'*'+series+'.csv'): 
                ...
                #batching
                a.append(torch.from_numpy(values))
        ...
        for item in all_frequencies:
            ...
            y_truth = torch.stack(a)[:,freq_indx].to(device)
...
def main():
    ...
    date="_RESNET152_Bands_5sep_2e-5_20epc_SGD_64_MSE_1out"
