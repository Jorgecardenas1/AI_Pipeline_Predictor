...
def arguments():
    ...
    parser.condition_len=5 #this is to be modified when training for different metrics.
...
def epoch_train(epoch,model,dataloader,validation_dataloader,device,opt,scheduler,criterion,clipEmbedder,df,run):
    ...
    with torch.no_grad():
        ...
        counter=0
        for ax in axs:
            ax.plot(range(0,100), y_predicted[counter].cpu().detach().numpy(),color="salmon")
...
def train(opt,scheduler,criterion,model, clipEmbedder,device, PATH, run):
    ...
    validation_dataloader = utils.get_data_with_labels(parser.image_size, parser.image_size,1,
...
def main():
    ...
    fwd_test, opt, criterion,scheduler=get_net_resnet(device,hiden_num=800,dropout=0.3,features=400, Y_prediction_size=parser.pred_size)
