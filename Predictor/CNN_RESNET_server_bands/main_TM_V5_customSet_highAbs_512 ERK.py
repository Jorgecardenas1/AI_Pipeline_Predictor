...
def get_net_resnet(device,hiden_num=1000,dropout=0.1,features=3000, Y_prediction_size=601):
    ...
    opt = optimizer.Adam(model.parameters(), lr=parser.learning_rate, betas=(0.5, 0.9),weight_decay=1e-5)
...
def epoch_train(epoch,model,dataloader,validation_dataloader,device,opt,scheduler,criterion,clipEmbedder,df,run):
    ...
    with torch.no_grad():
        ...
        loss = criterion(y_predicted.float(), aa.float()) 
        #Loss
    ...
    return i,epoch_loss,acc_train,score,running_val_loss
...
def train(opt,scheduler,criterion,model, clipEmbedder,device, PATH, run):
    ...
    for epoch in range(parser.epochs):
        ...
        total,epoch_loss,acc_train,score_train,running_val_loss=epoch_train(epoch,
                                                           model,
                                                           dataloader,
                                                           vdataloader,
                                                           device,
        ...
        valid_loss_list.append(running_val_loss/total)
...
def main():
    ...
    fwd_test, opt, criterion,scheduler=get_net_resnet(device,hiden_num=800,dropout=0.4,features=600, Y_prediction_size=parser.pred_size)
    ...
    """train"""
    loss_values,_,valid_loss_list,_,_=train(opt,
                                            scheduler,
                                            criterion,
                                            fwd_test,
                                            Bert,
